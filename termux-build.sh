...
while [ $# -gt 0 ]; do
  case $1 in
    --runtime) BUILD_SCRIPT="build.py"; shift ;;
    --no-deps) SKIP_DEPS=1; shift ;;
    --no-flash) SKIP_FLASH=1; shift ;;
    --yes|-y) ASSUME_YES=1; shift ;;
    -h|--help) usage 0 ;;
    --) shift; break ;;
    *) die "unknown option: $1 (build options go after --)" ;;
  esac
done
...
[ -f "$PROJECT_DIR/$BUILD_SCRIPT" ] || die "$PROJECT_DIR does not look like an MFFMv14 checkout ($BUILD_SCRIPT missing)"
...
{ python "$BUILD_SCRIPT" $INTERACTIVE_ARG "$@"; printf '%s\n' "$?" >"$BUILD_STATUS"; } | tee "$BUILD_LOG"
...
[ -f "$ZIP" ] || die "$BUILD_SCRIPT reported $ZIP but that file does not exist"
ok "$ZIP"
